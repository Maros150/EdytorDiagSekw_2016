...
/*	@Override
	...
	public PictogramElement add(IAddContext context) {
		...
		// TODO:
		edytorModel.Class addedClass = (edytorModel.Class) context.getNewObject();
        ...
        Image image;
        {
        	image = gaService.createImage(containerShape, EditorImageProvider.IMG_CLASS);
        	image.setStretchH(true);
        	image.setStretchV(true);
        	gaService.setLocationAndSize(image, context.getX(),context.getY() ,Helper.IMG_WIDTH_CLASS , Helper.IMG_HIGHT_CLASS); 
        ...
        // SHAPE WITH TEXT
        {
            ...
            text.setForeground(manageColor(Helper.TEXT_FOREGROUND_CLASS));
            text.setHorizontalAlignment(Orientation.ALIGNMENT_CENTER ); 
            // vertical alignment has as default value "center"
            text.setFont(gaService.manageDefaultFont(getDiagram(), false, true));
            gaService.setLocationAndSize(text, 0, 0, Helper.IMG_WIDTH_CLASS, Helper.IMG_HIGHT_CLASS);
            // create link and wire it
            ...
            // create and set text graphics algorithm
            text = gaService.createText(shape, Helper.TEXT_CLASS_TYPE);
            text.setForeground(manageColor(Helper.TEXT_FOREGROUND_CLASS));
            text.setHorizontalAlignment(Orientation.ALIGNMENT_CENTER ); 
            // vertical alignment has as default value "center"
            text.setFont(gaService.manageDefaultFont(getDiagram(), false, true));
	...
	public boolean canLayout(ILayoutContext context) {
		return context.getPictogramElement() instanceof ContainerShape  && getBusinessObjectForPictogramElement(context.getPictogramElement()) instanceof edytorModel.Class ;
	}
			...
			if (outerGraphicsAlgorithm instanceof Image) {
				...
				if (children.size() > 0) {
					...
					if (nameGraphicsAlgorithm instanceof Text && typeGraphicsAlgorithm instanceof Text) {
						Graphiti.getGaLayoutService().setLocationAndSize(nameGraphicsAlgorithm, 0, 0, roundedRectangle.getWidth(), Helper.TEXT_HIGHT_CLASS);
						Graphiti.getGaLayoutService().setLocationAndSize(typeGraphicsAlgorithm, 0, 0, roundedRectangle.getWidth(), Helper.IMG_HIGHT_CLASS);
						return true;
					}
				}
			}
	...
	public void preDelete(org.eclipse.graphiti.features.context.IDeleteContext context) {
        ...
        //lecimm po wszystkich ob typu cont i usuwamy co trzeba
	...
	public IReason updateNeeded(IUpdateContext context) {
	        ...
	        if (bo instanceof edytorModel.Class) {
	        	edytorModel.Class eClass = (edytorModel.Class) bo;
	...
	public boolean canUpdate(IUpdateContext context) {
        ...
        return (bo instanceof edytorModel.Class);
	}
	...
	private edytorModel.Class findElement(String name) {
		EList<EObject> contents = getDiagram().eResource().getContents();
		for (EObject eObject : contents) {
			if (eObject instanceof edytorModel.Class) {
				// TODO:
				if (name.equals(((edytorModel.Class) eObject).getName())) {
					return (edytorModel.Class) eObject;
				}
			}
		}
		return null;
